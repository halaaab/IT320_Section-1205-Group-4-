...
?>
<a class="order-row" href="provider-item-details.php?id=<?= htmlspecialchars((string)$item['_id']) ?>">
...
<div>
<div class="icon-label"><span>Quantity <span style="color:#d64545;">*</span></span></div>
<input class="form-input" type="number" name="quantity" id="quantity" min="1" placeholder="Enter quantity" min=1>
<div class="field-error" id="quantityError">Please enter a valid quantity.</div>
</div>
<div class="form-grid">
<div>
...
<span>Pickup Date <span style="color:#d64545;">*</span></span>
</div>
...
<div class="field-error" id="pickupDateError">
...
form.addEventListener('submit', function(e) {
  ...
  if (!itemPhotoInput.value && !photoUrlInput.value) {
  ...
  if (!pickupDate.value) {
    showError('pickupDateError');
    markField(pickupDate, true);
    valid = false;
  } else if (pickupDate.value < '2026-01-01') {
    showError('pickupDateError');
    markField(pickupDate, true);
    valid = false;
  } else {
    hideError('pickupDateError');
    markField(pickupDate, false);
  }
  ...
  if (photoUrlInput.value || !itemPhotoInput.files[0]) {
    return;
  }
