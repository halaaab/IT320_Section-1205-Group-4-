...
<div class="provider-block">
  ...
  <?php $loc = $g['location'] ?? []; $lat = $loc['lat'] ?? null; $lng = $loc['lng'] ?? null; ?>
  <div class="provider-location-wrap">
    <div class="provider-location-meta">
      <div class="provider-location-badge">📍 Provider pickup location</div>
      <h3><?= rp_h($loc['label'] ?? 'Main Branch') ?></h3>
      <p><?= rp_h($loc['text'] ?? 'Pickup location will be shown here.') ?></p>
      <p>This map is view-only and shows where the customer will collect items from this provider.</p>
    </div>
    <div>
      <?php if ($lat !== null && $lng !== null): ?>
        <iframe class="provider-map-frame" loading="lazy" referrerpolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q=<?= rp_h($lat) ?>,<?= rp_h($lng) ?>&hl=en&z=15&output=embed"></iframe>
      <?php else: ?>
        <div class="provider-map-fallback">Location coordinates are not available for this provider yet.</div>
      <?php endif; ?>
    </div>
  </div>
...
<?php endif; ?></div><?php rp_footer(); ?><script>
