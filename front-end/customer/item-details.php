...
if ($itemId) {
    ...
    if ($item) {
       ...
       if ($item) {
            ...
            // 1. Try the specific location assigned to the item
            $location = null;
            if (!empty($item['pickupLocationId'])) {
                $location = $locationModel->findById((string) $item['pickupLocationId']);
            }
            
            // 2. FALLBACK: Use the Provider's default location (Main Branch)
            if (empty($location) && !empty($item['providerId'])) {
                $location = $locationModel->getDefault((string) $item['providerId']);
            }
            
            // 3. LAST RESORT: Use the first location available for this provider
            if (empty($location) && !empty($item['providerId'])) {
                $allLocs = $locationModel->getByProvider((string) $item['providerId']);
                if (!empty($allLocs)) {
                    $location = (array) $allLocs[0];
                }
            }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nav-cart { width: 40px; height: 40px; border-radius: 50%; border: 2px solid rgba(255,255,255,.7); display: flex; justify-content: center; align-items: center; cursor: pointer; transition: background .2s; text-decoration: none; }
    .nav-cart:hover { background: rgba(255,255,255,.15); }
    .cart-badge { position: absolute; top: -5px; right: -5px; min-width: 19px; height: 19px; background: #e53935; border-radius: 50%; border: 2px solid #2255a4; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: #fff; font-family: 'DM Sans',sans-serif; pointer-events: none; animation: cartPop .4s cubic-bezier(.175,.885,.32,1.275); }
    ...
    .nav-center a { color: rgba(255,255,255,.85); text-decoration: none; font-weight: 500; font-size: 15px; font-family: 'Playfair Display',serif; transition: color .2s; }
    ...
    .nav-avatar { width: 38px; height: 38px; border-radius: 50%; border: 2px solid rgba(255,255,255,.6); display: flex; align-items: center; justify-content: center; cursor: pointer; background: rgba(255,255,255,.08); transition: background .2s; }
    .nav-avatar:hover { background: rgba(255,255,255,.2); }
    .btn-signup { background: #fff; color: #1a3a6b; border: none; border-radius: 50px; padding: 9px 22px; font-weight: 700; font-size: 14px; font-family: 'Playfair Display',serif; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,.1); transition: transform .15s,box-shadow .15s; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; }
    .btn-signup:hover { transform: translateY(-1px); box-shadow: 0 4px 16px rgba(0,0,0,.15); }
    .btn-login { background: transparent; color: #fff; border: 2px solid #fff; border-radius: 50px; padding: 8px 22px; font-weight: 700; font-size: 14px; font-family: 'Playfair Display',serif; cursor: pointer; transition: background .2s; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; }
    ...
    .bell-badge { position: absolute; top: -3px; right: -3px; min-width: 18px; height: 18px; background: #e53935; border-radius: 50%; border: 2px solid #2255a4; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: #fff; pointer-events: none; font-family: 'DM Sans',sans-serif; animation: cartPop .4s cubic-bezier(.175,.885,.32,1.275); }
    .notif-dropdown { display: none; position: absolute; top: 50px; right: 0; width: 360px; background: #fff; border-radius: 20px; box-shadow: 0 12px 48px rgba(26,58,107,.18); border: 1.5px solid #e0eaf5; z-index: 9999; overflow: hidden; }
    ...
    .notif-mark-all { font-size: 12px; color: #2255a4; background: none; border: none; cursor: pointer; font-family: 'Playfair Display',serif; font-weight: 600; padding: 0; transition: color .2s; }
    ...
    .hero-back { position: absolute; top: 20px; left: 24px; width: 46px; height: 46px; border-radius: 50%; border: none; background: #cdd9e8; display: grid; place-items: center; color: #1b3f92; font-size: 28px; font-weight: 700; cursor: pointer; text-decoration: none; transition: background .2s; line-height: 1; font-family: 'Playfair Display',serif; }
    .hero-back:hover { background: #bfcee2; }
    .hero-fav { position: absolute; top: 20px; right: 24px; width: 46px; height: 46px; border-radius: 50%; border: none; background: #cdd9e8; display: grid; place-items: center; font-size: 26px; cursor: pointer; transition: background .2s,transform .2s; }
    ...
    .hero-food-img { max-height: 270px; max-width: 480px; width: 100%; object-fit: contain; position: relative; z-index: 1; filter: drop-shadow(0 8px 20px rgba(26,58,107,.14)); }
    ...
    .qty-row { display: flex; align-items: center; background: #fff; border: 1.5px solid #dce7f5; border-radius: 50px; width: fit-content; overflow: hidden; box-shadow: 0 2px 8px rgba(26,58,107,.07); margin-bottom: 28px; }
    .qty-btn { width: 52px; height: 52px; border: none; background: transparent; font-size: 24px; font-weight: 700; color: #1a3a6b; cursor: pointer; transition: background .15s; display: grid; place-items: center; }
    ...
    .time-chip { padding: 11px 20px; border: 1.5px solid #dce7f5; border-radius: 14px; background: #fff; font-size: 15px; font-weight: 500; color: #1a2a45; cursor: pointer; transition: all .2s; user-select: none; }
    .time-chip.selected,.time-chip:hover { border-color: #1a3a6b; background: #e8f0ff; color: #1a3a6b; font-weight: 700; }
    .map-box { width: 100%; border-radius: 20px; overflow: hidden; border: 1.5px solid #dce7f5; height: 210px; margin-bottom: 8px; box-shadow: 0 4px 14px rgba(26,58,107,.08); display: block; cursor: pointer; }
    .map-box iframe { width: 100%; height: 100%; border: none; border-radius: 20px; display: block; }
    .map-box-placeholder { width: 100%; border-radius: 20px; overflow: hidden; border: 1.5px solid #dce7f5; background: linear-gradient(135deg,#c8dbf5,#dce7f5); height: 210px; display: flex; align-items: center; justify-content: center; margin-bottom: 8px; box-shadow: 0 4px 14px rgba(26,58,107,.08); }
    ...
    .detail-add-btn { background: #e88922; color: #fff; border: none; border-radius: 22px; padding: 16px 26px; width: 100%; max-width: 420px; display: flex; align-items: center; justify-content: space-between; font-family: 'Playfair Display',serif; font-size: 22px; font-weight: 700; cursor: pointer; box-shadow: 0 6px 16px rgba(224,122,26,.25); transition: .2s; }
    .detail-add-btn:hover:not(:disabled) { transform: translateY(-1px); }
    .detail-add-btn:disabled { background: #b0bec5; box-shadow: none; cursor: not-allowed; opacity: .7; transform: none; }
    ...
    .login-nudge { display: flex; align-items: center; gap: 14px; background: #fff8f0; border: 1.5px solid #f3c999; border-radius: 16px; padding: 16px 20px; margin-top: 16px; }
    .login-nudge-text { font-size: 14px; color: #7a5c2a; flex: 1; line-height: 1.5; }
    .login-nudge-text strong { color: #c06a10; }
    .login-nudge-btns { display: flex; gap: 8px; flex-shrink: 0; }
    .nudge-login-btn { background: #1a3a6b; color: #fff; border: none; border-radius: 50px; padding: 10px 22px; font-size: 14px; font-weight: 700; font-family: 'Playfair Display',serif; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; }
    .nudge-signup-btn { background: transparent; color: #1a3a6b; border: 2px solid #1a3a6b; border-radius: 50px; padding: 9px 18px; font-size: 14px; font-weight: 700; font-family: 'Playfair Display',serif; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; }
    ...
    .footer-bottom { display: flex; align-items: center; gap: 8px; color: rgba(255,255,255,.7); font-size: 13px; font-family: 'Playfair Display',serif; }
    ...
    .mobile-menu a { color:rgba(255,255,255,.85); font-size:22px; font-weight:700; font-family:'Playfair Display',serif; padding:18px 0; border-bottom:1px solid rgba(255,255,255,.12); text-decoration:none; }
    ...
    .mobile-search-dropdown { display:none; background:#fff; border-radius:16px; margin:0 0 8px; overflow:hidden; max-height:55vh; overflow-y:auto; box-shadow:0 4px 24px rgba(26,58,107,.18); }
    .mobile-search-dropdown.open { display:block; }
    .mobile-search-dropdown a.search-item-row { color:#1a3a6b!important; font-size:14px!important; padding:10px 16px!important; border-bottom:1px solid #f5f8fc!important; display:flex!important; align-items:center!important; gap:12px!important; background:#fff!important; }
    ...
    .mobile-search-dropdown .search-item-sub { font-size:12px!important; color:#7a8fa8!important; }
    ...
    .mobile-search-dropdown .search-section-label { background:#fff; padding:10px 16px 6px!important; font-size:11px!important; font-weight:700!important; color:#b0c4d8!important; text-transform:uppercase!important; }
...
@media(max-width:768px) {
      ...
      /* Item Details Mobile Scaling */
      .item-hero { padding: 16px 16px 0; min-height: 240px; }
      .hero-back, .hero-fav { width: 38px; height: 38px; font-size: 22px; top: 16px; }
      .hero-back { left: 16px; }
      .hero-fav { right: 16px; }
      .prov-hero-logo { height: 42px; bottom: 16px; left: 16px; }
      .prov-hero-name { font-size: 13px; bottom: 16px; left: 16px; }
      .hero-food-img { max-height: 200px; max-width: 90%; }
      
      .container { padding: 24px 16px 32px; }
      .item-title { font-size: 24px; }
      .item-price-big { font-size: 22px; }
      .riyal-icon-top { width: 22px; height: 22px; }
      .item-ingredients { font-size: 14px; margin-bottom: 12px; }
      .expiry-row { font-size: 13px; margin-bottom: 16px; }
      .divider { margin: 16px 0; }
      .section-label { font-size: 18px; margin-bottom: 12px; }
      
      .qty-btn { width: 44px; height: 44px; font-size: 20px; }
      .qty-val { min-width: 48px; font-size: 18px; height: 44px; }
      .time-chip { padding: 8px 16px; font-size: 14px; }
      .map-box, .map-box-placeholder { height: 160px; }
      
      .detail-add-btn { padding: 14px 20px; border-radius: 50px; }
      .btn-text { font-size: 18px; }
      .btn-price { font-size: 18px; }
      .riyal-icon { width: 18px; height: 18px; }

      .login-nudge { flex-direction: column; align-items: flex-start; gap: 12px; padding: 14px 16px; }
      .login-nudge-text { font-size: 13px; }
      .login-nudge-btns { width: 100%; }
      .nudge-login-btn, .nudge-signup-btn { flex: 1; justify-content: center; padding: 10px 0; }
  ...
  <div class="nav-right">
    ...
    <div id="authModal" style="display:none;position:fixed;inset:0;background:rgba(12,22,45,0.5);z-index:9999;justify-content:center;align-items:center;" onclick="if(event.target===this)this.style.display='none'">
      <div style="background:#fff;border-radius:24px;padding:44px 40px;max-width:400px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.2);animation:floatUp 0.3s ease;">
        ...
        <p style="font-size:14px;color:#7a8fa8;margin-bottom:28px;line-height:1.6;">Log in or create an account to add items to your cart and place orders.</p>
...
<div class="item-hero">
  ...
  <?php else: ?>
    <button class="hero-fav" type="button" onclick="document.getElementById('authModal').style.display='flex'" style="border:none;cursor:pointer;">
...
<main class="container">
  ...
  <?php if ($justAdded): ?>
    <div class="flash">&#10003; Added to cart! <a href="cart.php" style="color:#1a3a6b;font-weight:700;">View cart &rarr;</a></div>
  <?php endif; ?>
  ...
  <?php if (!empty($pickupTimes)): ?>
    ...
    <div class="pickup-times" style="margin-bottom:28px;<?= !$isLoggedIn ? 'opacity:.55;pointer-events:none;' : '' ?>">
  ...
  <?php if ($isLoggedIn): ?>
    <form method="post" id="cartForm">
      ...
      <div class="detail-add-btn-wrap">
        <button class="detail-add-btn" type="submit" form="cartForm" <?= !$isAvailable ? 'disabled' : '' ?>>
          <span class="btn-text"><?= !$isAvailable ? 'Unavailable' : ($inCart ? 'Update cart' : 'Add to cart') ?></span>
          <?php if ($isAvailable): ?>
            <?php if (!$isFree): ?>
              <span class="btn-price">
                <span id="btnTotal"><?= number_format($price, 2) ?></span>
                <img src="../../images/SAR.png" class="riyal-icon">
              </span>
            <?php else: ?>
  ...
  <?php else: ?>
    <!-- Guest: button always visible, opens auth modal on click -->
    <div class="divider"></div>
    <div class="detail-add-btn-wrap">
      <button class="detail-add-btn" type="button" onclick="document.getElementById('authModal').style.display='flex'">
    ...
    <div class="login-nudge">
      <svg width="22" height="22" fill="none" stroke="#c06a10" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
      <p class="login-nudge-text"><strong>Log in or sign up</strong> to add this item to your cart and place an order.</p>
      <div class="login-nudge-btns">
        <a href="../shared/login.php" class="nudge-login-btn">Log in</a>
        <a href="../shared/signup-customer.php" class="nudge-signup-btn">Sign up</a>
      </div>
    </div>
  <?php endif; ?>
